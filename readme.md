...
`jazztun` builds a TCP tunnel over Salute Jazz WebRTC DataChannel sessions.
In practice, the client exposes a local SOCKS5 proxy and forwards TCP traffic through one or more Salute Jazz rooms to a remote server process.
...
- Multi-room sharding with `-rooms N`
...
- Best throughput comes from multiple concurrent TCP connections, rooms, and peers, not from one single stream
...
#### Option 1: Let The Server Create Room(s)

Start the server first:

```bash
./server -room new -rooms 2 -peers 4 -v
```

The server creates one or more rooms, generates a key if `-key` is omitted, and prints a copy-paste-ready room list and key.

Start the client with the same room URL and key:

```powershell
.\client.exe -room "https://salutejazz.ru/ROOM1?psw=...,https://salutejazz.ru/ROOM2?psw=..." -key YOUR_KEY -peers 4 -v
```

The local SOCKS5 proxy listens on `127.0.0.1:1080` by default.

#### Option 2: Join Existing Room(s)

```bash
./server -room "https://salutejazz.ru/ROOM1?psw=...,https://salutejazz.ru/ROOM2?psw=..." -key YOUR_KEY -rooms 2 -peers 4
./client -room "https://salutejazz.ru/ROOM1?psw=...,https://salutejazz.ru/ROOM2?psw=..." -key YOUR_KEY -rooms 2 -peers 4
```
...
- `-room`: room URL list or `new`
- `-key`: 64-character hex key; autogenerated if omitted on the server
- `-rooms`: number of rooms to create with `-room new`, or expected number of room URLs in `-room`
...
- `-room`: room URL or comma-separated room URL list
- `-key`: same 64-character hex key as the server
- `-listen`: local SOCKS5 listen address, default `127.0.0.1:1080`
- `-rooms`: expected number of room URLs in `-room` (optional validation)
- `-duo`: shorthand for two transport peers
- `-peers`: number of transport peers to open; overrides `-duo`
- `-v`: verbose logging
...
`jazztun` строит TCP-туннель поверх комнат Salute Jazz и WebRTC DataChannel.
На практике клиент поднимает локальный SOCKS5-прокси и пересылает TCP-трафик через одну или несколько комнат Salute Jazz на удаленный серверный процесс.
...
- multi-room sharding через `-rooms N`
...
- максимальная пропускная способность лучше раскрывается на нескольких параллельных TCP-соединениях, нескольких комнатах и peer-ах, а не на одном потоке
...
#### Вариант 1: Сервер Сам Создает Комнаты
...
Сервер создаст одну или несколько комнат, а если `-key` не задан, сам сгенерирует ключ и выведет room list и ключ.
...
#### Вариант 2: Обе Стороны Входят В Уже Существующие Комнаты
...
- `-room`: room list или `new`
- `-key`: 64-символьный hex-ключ; на сервере может быть сгенерирован автоматически
- `-rooms`: количество комнат для `-room new` или ожидаемое число room URL в `-room`
